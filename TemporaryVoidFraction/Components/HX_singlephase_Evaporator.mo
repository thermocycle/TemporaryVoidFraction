...
model HX_singlephase_Evaporator
  "HX singlephase model compatible with MSL - countercurrent configuration only"
  replaceable package MediumPs =ThermoCycle.Media.R600a_CP  constrainedby
  ...
  replaceable package MediumSs =ThermoCycle.Media.Air_CP  constrainedby
...
protected
  ...
  /*Metal Wall*/
public
  parameter Modelica.SIunits.Mass M_wall= 1.03 "Mass of the Wall";
  parameter Modelica.SIunits.SpecificHeatCapacity c_wall= 500
    "Specific heat capacity of the metal wall";

  parameter Modelica.SIunits.Mass M_ps = 0.010 "Mass of the fluid"
                        annotation (Dialog(group="Primary side", tab="General"));
  parameter Modelica.SIunits.MassFlowRate MdotNom_ps = 0.0003
    "Nominal mass flow rate" annotation (Dialog(group="Primary side", tab="General"));

  parameter Modelica.SIunits.Mass M_ss = 0.10 "Mass of the fluid"
                        annotation (Dialog(group="Secondary side", tab="General"));
  parameter Modelica.SIunits.MassFlowRate MdotNom_ss = 0.005
  ...
  parameter Modelica.SIunits.Area A_ps=0.160 "Heat transfer area primary side" annotation (Dialog(group="Heat transfer", tab="General", enable=(not Use_AU)));
  parameter Modelica.SIunits.Area A_ss=1.10 "Heat transfer area secondary side"                         annotation (Dialog(group="Heat transfer", tab="General", enable=(not Use_AU)));
  parameter Modelica.SIunits.CoefficientOfHeatTransfer U_ps = 340
    "Constant heat transfer coefficient primary side" annotation (Dialog(group="Heat transfer", tab="General"));
  parameter Modelica.SIunits.CoefficientOfHeatTransfer U_ss = 20
  ...
  //MediumPs.SaturationProperties satProperties; //ADDED BY MARCO DINIZ
...
equation
  /* Added by Marco Diniz to help during off period - Allows separation 0f condenser and evaporator temperatures but seems unstable */
//     satProperties = MediumPs.setSat_T(T_wall);
//     if InFlowPs.m_flow>MdotNom_ps/100 then
//     stateIn_ps = MediumPs.setState_ph(InFlowPs.p,inStream(InFlowPs.h_outflow));
//     else
//     stateIn_ps = MediumPs.setState_ph(satProperties.psat,h_ss_ex);
//     end if;
    ...
    stateOut_ss = MediumSs.setState_ph(OutFlowSs.p,h_ss_ex);
     ...
     if not Use_AU then
      AU_ps = A_ps * U_ps * (max(MdotNom_ps/100,InFlowPs.m_flow) / MdotNom_ps)^(0.8); //MODIFIED BY MARCO DINIZ;
      AU_ss = A_ss * U_ss * (max(MdotNom_ss/100,InFlowSs.m_flow) / MdotNom_ss)^(0.8); //MODIFIED BY MARCO DINIZ;
      A = (A_ps+A_ss)/2 "dummy";
    else
      AU_global = 1 / (1/AU_ps + 1/AU_ss);
      AU_ps = A * U_ps * (max(MdotNom_ps/100,InFlowPs.m_flow) / MdotNom_ps)^(0.8); //MODIFIED BY MARCO DINIZ
      AU_ss = A * U_ss * (max(MdotNom_ss/100,InFlowSs.m_flow) / MdotNom_ss)^(0.8); //MODIFIED BY MARCO DINIZ
    end if;
